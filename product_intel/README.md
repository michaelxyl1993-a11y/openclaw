...
## v1.26 Final Feishu File Messages

v1.26 sends the uploaded final attachments as real Feishu file messages.

- Real file send is disabled unless `PRODUCT_INTEL_REAL_FEISHU_FILE_SEND_ENABLED=true`.
- The public upload receipt contains only redacted file token hashes.
- Raw upload file tokens are read from the gitignored private secure receipt generated by v1.25 upload.
- File messages use `POST /open-apis/im/v1/messages?receive_id_type=chat_id` with `msg_type=file` and `content={"file_key": "<uploaded file token>"}`.

If the secure receipt is missing, rerun v1.25 upload after this version so it can create:

```text
product_intel/private_runtime/feishu_final_attachment_upload_secure_receipt.json
```

Send the uploaded files as file messages:

```bash
PRODUCT_INTEL_REAL_FEISHU_FILE_SEND_ENABLED=true \
FEISHU_APP_ID="$FEISHU_APP_ID" \
FEISHU_APP_SECRET="$FEISHU_APP_SECRET" \
python3 -m product_intel.feishu_final_attachment_file_send_real \
  --upload-receipt product_intel/output_feishu_llm_ops/feishu_final_attachment_upload_receipt.json \
  --chat-id "$PRODUCT_INTEL_FEISHU_TEST_CHAT_ID" \
  --output-dir product_intel/output_feishu_llm_ops
```
